 ...
 * token urls on the target workflow instance. Always provides REFRESHable tokens (see
...
public class HttpFinbourneTokenProvider {
    ...
    /** configuration parameters to connect to workflow */
    private final ApiConfiguration apiConfiguration;

    /** client to make http calls to workflow */
    ...
    /**
     * Retrieves a {@link FinbourneToken} via an authentication call to workflow.
     *
    ...
    private FinbourneToken callAndMapResponseToToken(OkHttpClient httpClient, Request request) throws FinbourneTokenException {
        ...
        if (response.code() != 200) {
            throw new FinbourneTokenException("Authentication call to workflow failed. See response :" + response.toString());
        }
        ...
        try {
            ...
            throw new FinbourneTokenException("Failed to correctly map the authentication response from workflow. See details : ", e);
        }

        if (!bodyValues.containsKey("access_token")) {
            throw new FinbourneTokenException("Response from workflow authentication is missing an access_token entry");
        }

        if (!bodyValues.containsKey("refresh_token")) {
            throw new FinbourneTokenException("Response from workflow authentication is missing an refresh_token entry");
        }

        if (!bodyValues.containsKey("expires_in")) {
            throw new FinbourneTokenException("Response from workflow authentication is missing an expires_in entry");
        }
