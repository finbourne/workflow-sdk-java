package com.finbourne.workflow.extensions.auth;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.finbourne.workflow.extensions.ApiConfiguration;
import okhttp3.*;
 ...
 * token urls on the target lusidWorkflow instance. Always provides REFRESHable tokens (see
...
public class HttpFinbourneTokenProvider {
    ...
    /** configuration parameters to connect to lusid-workflow */
    private final ApiConfiguration apiConfiguration;

    /** client to make http calls to lusid-workflow */
    ...
    /**
     * Retrieves a {@link FinbourneToken} via an authentication call to lusid-workflow.
     *
    ...
    private FinbourneToken callAndMapResponseToToken(OkHttpClient httpClient, Request request) throws FinbourneTokenException {
        ...
        if (response.code() != 200) {
            throw new FinbourneTokenException("Authentication call to lusidWorkflow failed. See response :" + response.toString());
        }
        ...
        try {
            ...
            throw new FinbourneTokenException("Failed to correctly map the authentication response from lusidWorkflow. See details : ", e);
        }

        if (!bodyValues.containsKey("access_token")) {
            throw new FinbourneTokenException("Response from lusidWorkflow authentication is missing an access_token entry");
        }

        if (!bodyValues.containsKey("refresh_token")) {
            throw new FinbourneTokenException("Response from lusidWorkflow authentication is missing an refresh_token entry");
        }

        if (!bodyValues.containsKey("expires_in")) {
            throw new FinbourneTokenException("Response from lusidWorkflow authentication is missing an expires_in entry");
        }
